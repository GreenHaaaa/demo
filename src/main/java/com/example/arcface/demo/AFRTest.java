package com.example.arcface.demo;

import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.io.InputStream;
import com.example.arcface.utils.BufferInfo;
import com.example.arcface.utils.ImageLoader;
import com.example.arcface.*;
...
//        getsimilarity("E:\\tools\\2.png");
    ...
    public static float authFromFeatrue(AFR_FSDK_FACEMODEL fsdk_facemodel,String path)
    {
        ...
        NativeLong rets = AFR_FSDKLibrary.INSTANCE.AFR_FSDK_FacePairMatching(hFREngine, faceFeatureA, fsdk_facemodel, fSimilScore);

        return fSimilScore.getValue();
    }
    public static String  getsimilarity(String args) {
        System.out.println("#####################################################");
    ...
    public static byte[] getFeature(String path)
    {
        /* init Engine */
        Pointer pFDWorkMem = CLibrary.INSTANCE.malloc(FD_WORKBUF_SIZE);
        Pointer pFRWorkMem = CLibrary.INSTANCE.malloc(FR_WORKBUF_SIZE);
        ...
        AFR_FSDK_FACEMODEL faceFeature = extractFRFeature(hFREngine,inputImg,faceInfos[0]);
        try {
            System.out.println(faceFeature.lFeatureSize);
            return faceFeature.toByteArray();
        }catch (Exception e)
        {
            e.printStackTrace();
        }
